#include <stdio.h>
                                                                               #define N_COLUMN 50 /* character limit for breaking column */
...
int main(void)
{
int in_cnt, out_cnt; /* input and output char counting variables */ int in_size; /* actual input size */
...
i = 0; while(i < size-1 && (c = getchar()) != EOF && c != '\n')
{
line[i] = c;
i++;
}
